...
async def get_all_templates(user=Depends(get_user), db: Session = Depends(get_db)):
...
async def get_template_by_id(
    template_id: str, user=Depends(get_user), db: Session = Depends(get_db)
):
...
async def update_template(
    template_id: str, admin=Depends(get_admin_user), db: Session = Depends(get_db)
):
